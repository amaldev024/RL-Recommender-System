...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, name='DQNetwork'):
        ...
        with tf.variable_scope(self.name):
            ...
            self.negative=tf.placeholder(tf.int32,[None])
            ...
            qloss, q_learning = trfl.double_qlearning(self.output, self.actions, self.reward, self.discount,
                                                      self.targetQs_, self.targetQs_selector)
            q_indexed_positive = indexing_ops.batched_index(self.output, self.actions)
            q_indexed_negative=indexing_ops.batched_index(self.output,self.negative)
            BPR_loss=-tf.log(tf.sigmoid(q_indexed_positive-q_indexed_negative))
            self.loss = tf.reduce_mean(qloss+BPR_loss)
